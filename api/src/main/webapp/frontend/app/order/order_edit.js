...
_.extend(App.OrderController.prototype, Backbone.Events, {
    ...
    refreshMetaAndGetDrivers: function () {
        ...
        $.ajax(CONTEXT_PATH + '/api/order/routeMeta', {
            context: this,
    ...
    updateDriversList: function () {
        ...
        if (this.order.get('routeLength') != null && this.trucksCollection.length > 0) {
            ...
            $.ajax(CONTEXT_PATH + "/api/order/drivers", {
                    context: this,
                    data: {
    ...
    onSubmitClicked: function () {
        ...
        $.ajax(CONTEXT_PATH + "/api/order/edit" + (id != null ? '?id=' + id : ''), {
            context: this,
            method: "POST",
        ...
        .done(function () {
            ...
            setTimeout(function(){
                window.location.replace(CONTEXT_PATH + "/order/list");
            }, 500);
        })
        .fail(function (jqXhr) {
            showAlert("ajax error: " + jqXhr.errorText, 'danger');
